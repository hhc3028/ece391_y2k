...
struct terminal_struct terminals[NUM_TERMINAL];
...
void keyboard_getchar()
{
	...
	case(F2):
		if(alt_flag == 1)	
			switch_terminal(&terminals[1]);
		break;
	case(F3):
		if(alt_flag == 1)
			switch_terminal(&terminals[2]);
		break;
	case(CAPS): //set flag
		if(c_flag == 0)
		{
...
uint32_t init_terminals(void)
{
	...
	{
		terminals[x].terminal_id = x;
		terminals[x].read_flag = 0;
		terminals[x].x_coordinate = 0;
		terminals[x].y_coordinate = 0;

		for(y = 0; y < BUF_MAX; y ++)
		{
			terminals[x].keyboard_buf[y] = 0;
		}

		terminals[x].video_page = vir_vid + (_4KB * x);
		terminals[x].storage_page = video_memory[x];
...
void switch_terminal(terminal_struct_t* terminal)
{
	...
	flush_tlb();
	memcpy((void *)cur_terminal->video_page, (void *)vir_vid, 80*25*2);

	cur_terminal = terminal;	// the current terminal points to new terminal
	memcpy((void *) vir_vid, (void *) cur_terminal->video_page, 80*25*2);
